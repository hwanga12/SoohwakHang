...
def test_farm_mapping_patrol_plan_stays_within_rectangular_world_bounds() -> None:
    ...
    assert len(plan.default_patrol_sequence) == 9
...
def test_farm_mapping_patrol_plan_starts_from_spawn_aligned_center_sweep() -> None:
    ...
    assert plan.waypoints['farm_01_lane_02_north'].pose.x == 8.6
    assert plan.waypoints['farm_01_lane_02_north'].pose.y == 4.0
    assert plan.default_patrol_sequence[:4] == (
        'farm_01_home',
        'farm_01_lane_01_south',
        'farm_01_lane_01_north',
        'farm_01_lane_02_north',
    )
